...
// Is a Behemoth or Leviathan attack on? The galaxy script behemothevent.lua
// keeps its state in a table that its secure() hands out (read on a running
// server 21.09.2026). While an attack is on, it holds
//   currentlyAttackedSector = { coords = {x=, y=}, quadrant = n }
// (Leviathans - Behemoths Rework 3370881698, behemothevent.lua lines 87-89).
// That entry stays until the attack window runs out, NOT until the boss dies
// (lines 53-71) - so it tells whether an attack is on, not whether the boss
// is still alive. Anything unreadable says "?" rather than "nil".
function bossCommand(marker) {
  return `/run print((function() local r,d=Galaxy():invokeFunction("data/scripts/galaxy/behemothevent.lua","secure") if r~=0 or type(d)~="table" then return "${marker} off" end local s,p=d.currentlyAttackedSector,"" if s then p="?" pcall(function() local c=s.coords or s local x,y=c.x or c[1],c.y or c[2] if x and y then p=x..":"..y end end) end return "${marker} "..tostring(d.countDown).." "..p end)())`;
}

// "off" | "<countDown> <where>" - where is empty when no attack is on, "?"
...
function formatBoss(state) {
  ...
  if (state.off) return 'Leviathan attacks are not running on this server.';
  // "attack", not "boss": the entry outlives the boss until the window closes
  if (state.sector) return `A Leviathan attack is on at (${state.sector.x}:${state.sector.y}).`;
  if (state.out) return 'A Leviathan attack is on right now.';
  return 'No Leviathan attack right now.';
}
