...
function formatBoss(state) {
  ...
  if (state.sector) return `A boss attack is on at (${state.sector.x}:${state.sector.y}).`;
  if (state.out) return 'A boss attack is on right now.';
  return 'No boss attack right now.';
}
