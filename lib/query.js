...
// while nothing was out. What that sector looks like WHILE a boss is out has
// not been seen yet, so both shapes are read ({x=,y=} and {1,2}), and one that
// cannot be read says "?" instead of pretending nothing is there.
function bossCommand(marker) {
  return `/run print((function() local r,d=Galaxy():invokeFunction("data/scripts/galaxy/behemothevent.lua","secure") if r~=0 or type(d)~="table" then return "${marker} off" end local s,x,y=d.currentlyAttackedSector,"","" if s then x,y="?","?" pcall(function() x=tostring(s.x or s[1]) y=tostring(s.y or s[2]) end) end return "${marker} "..tostring(d.countDown).." "..x.." "..y end)())`;
}

// "off" | "<countDown> <x> <y>" (x and y empty when nothing is out)
function parseBoss(answer) {
  if (answer === null || answer === undefined) return null;
  const text = String(answer).trim();
  if (text === 'off') return { off: true };
  const [count, x = '', y = ''] = String(answer).split(' ');
  const out = { off: false, countDown: Number(count), sector: null };
  if (x !== '' || y !== '') out.sector = { x, y };
  return out;
}

// Minutes are left out on purpose: what countDown means while a boss is out
// is read from the script, not measured. The sector is what players need.
